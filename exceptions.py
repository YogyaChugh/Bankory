...
class PlayerPoor(Exception):
    def __init__(self):
        ...
        """
        self.message = f"""Player
        {os.environ.get('current_player')} has gone in debt !"""
        super.__init__(self.message)
...
class GameFunctioningException(Exception):
    def __init__(self, msg):
        """
        Issue in functioning of game ranging from
        missing/corrupt files to incorrect env vars to
        unintended usages
        """
        self.message = msg
        super().__init__(self.message)
...
class OwnershipException(Exception):
    def __init__(self, property_card):
        ...
        """
        super().__init__()
        self.message = f"""Property Card {property_card.name}
            already owned by {property_card.owner}"""
