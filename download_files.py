"""
    I have no idea about how this works ! It's just taken from google and AI !
    It's download() function downloads the requested file/folder at the destination location provided !
     
    Rest all code/files (except 3 functions) written by @YogyaChugh :) - I love self-written code ! That's why !
"""
...
def make_shareable(service, file_id):
    try:
        ...
        raise GameFunctioningException(f"Couldn't set permissions of file ! {file_id}")
...
def download(remote_path, local_destination):
    ...
    if not file_id:
        raise GameFunctioningException(f"Folder/File not found at {remote_path}")
